...
export function RegistrationsList() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s.includes('approved') || s === 'paid') {
      return <span className="text-green-500 flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Pago</span>;
    }
    if (s.includes('pending')) {
      return <span className="text-yellow-500 flex items-center gap-1"><Clock className="w-3 h-3"/> Pendente</span>;
    }
    return <span className="text-red-500 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> {status}</span>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <CardTitle className="text-white">Histórico Geral ({filteredRegistrations.length})</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm text-left text-slate-400">
              ...
              <tbody className="divide-y divide-slate-800 bg-slate-900">
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="p-8 text-center text-slate-500">Nenhuma inscrição encontrada.</td></tr>
                ) : (
                    ...
                    <tr key={reg.id} className="hover:bg-slate-800/50">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-white font-medium">{reg.user.username}</div>
